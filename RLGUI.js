...
class RLGUI extends GUI {
    ...
    setMap() {
        ...
        let learnType = document.getElementById('selectlearn');
        let learn = (learnType==null) ? 'Q': learnType.value;
        ...
        else if (map=='cliff')      { this.env = new Environment(cliffMap);}
        this.sqSize = this.pixelWidth / this.env.width;
        if(learn=="Q"){
            this.RL = new RL_Q(this.env, this.getConfig());
        }
        else if(learn == "SARSA"){
            this.RL = new RL_SARSA(this.env, this.getConfig());
            this.bannerDiv.innerHTML = "<b>GridWorld SARSA - <a href='http://www.cs.mun.ca/~dchurchill/'>David Churchill</a>";
        }
        else if(learn == "EV_MC"){
    ...
    drawPolicyLine(x, y, a, p) {
        ...
        let px = parseInt((x + 0.5) * this.sqSize);
        let py = parseInt((y + 0.5) * this.sqSize);
        let width = this.sqSize*a[0] * p * 0.5;
        let height = this.sqSize*a[1] * p * 0.5;
    ...
    addEventListeners() {
        ...
        this.fg.addEventListener('mousedown', function (evt) {
            ...
            if (evt.which == 3) {
                ...
                if(this.gui.RL.type == "SARSA"){
                    this.gui.RL.a = this.gui.RL.selectActionFromPolicy([x, y]); 
                }
    ...
    setHTML() {
        ...
        this.addSelectBox(this.controlDiv, 'selectmap', c2left, top + s++*skip, 250, 25, function() { this.gui.setMap(); }, 
            [['default', 'Default Environment'], ['mini', 'Mini Environment'], ['tiny', 'Tiny Environment'], ['large', 'Large Environment'], ['cliff', 'Cliff Environment']]);
