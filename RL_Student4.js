...
class RL_FV_MC {
    ...
    learningIteration() {
        while(this.env.isTerminal(this.state[0], this.state[1])||this.env.isBlocked(this.state[0], this.state[1])){
            this.updateValue();
            this.updatePolicy(this.record[0]);
        ...
        this.record[0].push(this.state); this.record[1].push(a); this.record[2].push(R);
    ...
    updateValue() {
        let returns=0;
        loop1: for(let i=this.record[0].length-1; i>-1; i--){
            let state = this.record[0][i];
            let action = this.record[1][i];
            for(let j=i-1; j>-1; j--){
                if(this.record[0][j][0] == state[0] && this.record[0][j][1] == state[1] && this.record[1][j] == action){
                    continue loop1;
                }
            }    
           
            returns += this.record[2][i];
            this.Q[state[0]][state[1]][action] = this.Q[state[0]][state[1]][action] + this.config.alpha*(returns- this.Q[state[0]][state[1]][action]);
